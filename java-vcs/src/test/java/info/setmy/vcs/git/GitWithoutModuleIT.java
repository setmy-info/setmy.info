...
public class GitWithoutModuleIT {
    ...
    public void before() throws IOException {
        ...
        vcsA = VcsFactory.instanceOf(REPO_TYPE, URL_A, DIR);
        vcsB = VcsFactory.instanceOf(REPO_TYPE, URL_B, DIR);
    }
    ...
    public void initial_directories() {
        ...
        assertThat(vcsA.getWorkingDirectory().getAbsolutePath()).endsWith("/java-vcs/target/stealer-test-a");
        assertThat(vcsB.getCloneWorkingDirectory().getAbsolutePath()).endsWith("/java-vcs/target");
        assertThat(vcsB.getWorkingDirectory().getAbsolutePath()).endsWith("/java-vcs/target/stealer-test-b");
    }
    ...
    public void initial_directoriesWin() {
        ...
        assertThat(vcsA.getWorkingDirectory().getAbsolutePath()).endsWith("\\java-vcs\\target\\stealer-test-a");
        assertThat(vcsB.getCloneWorkingDirectory().getAbsolutePath()).endsWith("\\java-vcs\\target");
        assertThat(vcsB.getWorkingDirectory().getAbsolutePath()).endsWith("\\java-vcs\\target\\stealer-test-b");
    }
